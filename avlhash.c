...
struct avl_hash_node *avl_hash_node_next(
    struct avl_hash_table *ht,
...
struct avl_hash_node *avl_hash_node_prev(
    struct avl_hash_table *ht,
    struct avl_hash_node *node
) {
    struct avl_node *avlnode;
...
//---------------------------------------------------------------------
struct avl_hash_node *avl_hash_find(
    struct avl_hash_table *ht,
...
struct avl_node **avl_hash_track(
    struct avl_hash_table *ht,
    const struct avl_hash_node *node,
...
struct avl_hash_node *avl_hash_add(
    struct avl_hash_table *ht,
    struct avl_hash_node *node
) {
...
void avl_hash_replace(
    struct avl_hash_table *ht,
    struct avl_hash_node *victim,
    struct avl_hash_node *newnode
) {
...
void avl_hash_clear(
    ...
    while (!ilist_is_empty(&ht->head)) {
        struct avl_hash_index *index =
            ilist_entry(ht->head.next, struct avl_hash_index, node);
...
void *avl_hash_swap(struct avl_hash_table *ht, void *ptr, size_t nbytes) {
    ...
    while (!ilist_is_empty(&head)) {
        struct avl_hash_index *index =
            ilist_entry(head.next, struct avl_hash_index, node);
#if 1
        struct avl_node *next = NULL;
...
struct avl_hash_entry *avl_map_next(
...
struct avl_hash_entry *avl_map_prev(
    struct avl_hash_map *hm,
    struct avl_hash_entry *n
) {
...
static inline struct avl_hash_entry *avl_hash_entry_allocate(
    struct avl_hash_map *hm,
    void *key,
    void *value
) {
    ...
    ASSERTION(entry);
    if (hm->key_copy)
        entry->node.key = hm->key_copy(key);
    else
        entry->node.key = key;
    if (hm->value_copy)
...
static inline struct avl_hash_entry *avl_hash_update(
    struct avl_hash_map *hm,
    void *key,
    void *value,
    int update
...
struct avl_hash_entry *avl_map_add(
    struct avl_hash_map *hm,
    void *key,
    void *value,
    int *success
...
struct avl_hash_entry *avl_map_set(
    struct avl_hash_map *hm,
    void *key,
    void *value
) {
