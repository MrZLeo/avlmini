...
static inline void _avl_child_replace(
    struct avl_node *oldnode,
...
static inline struct avl_node *_avl_node_rotate_left(
...
static inline struct avl_node *_avl_node_rotate_right(
    struct avl_node *node,
    struct avl_root *root
) {
...
static inline void _avl_node_height_update(struct avl_node *node) {
    int h0 = AVL_LEFT_HEIGHT(node);
    int h1 = AVL_RIGHT_HEIGHT(node);
    node->height = AVL_MAX(h0, h1) + 1;
}

static inline struct avl_node *_avl_node_fix_l(
    struct avl_node *node,
    struct avl_root *root
) {
    struct avl_node *right = node->right;
...
static inline struct avl_node *_avl_node_fix_r(
    struct avl_node *node,
    struct avl_root *root
) {
    struct avl_node *left = node->left;
...
static inline void _avl_node_rebalance(
    struct avl_node *node,
    struct avl_root *root
) {
    while (node) {
