...
typedef struct ILISTHEAD ilist_head;
    ...
    #define ILIST_HEAD(name) struct ILISTHEAD name = ILIST_HEAD_INIT(name)

    #define ILIST_INIT(ptr) ((ptr)->next = (ptr), (ptr)->prev = (ptr))
...
// setup key
//---------------------------------------------------------------------
static inline void avl_hash_node_key(
    struct avl_hash_table *ht,
    struct avl_hash_node *node,
    void *key
) {
    ...
    do {                                                                       \
        ...
        while (__anode) {                                                      \
            ...
            if (__hash == __shash) {                                           \
                ...
                if (__hc == 0) {                                               \
                    (result) =                                                 \
                        AVL_ENTRY(__snode, struct avl_hash_entry, node);       \
                    break;                                                     \
                }                                                              \
